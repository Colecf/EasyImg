...
void writeBMP(string filename, unsigned char** pixels, uint32_t w, uint32_t h) {
  ...
  writeUint16(header+28, 24);  //bpp, using 1 char per pixel means 8
  ...
  for(int row=0; row<h; row++) {
    int col;
    for(col=0; col<w; col++) {
      outfile.write((const char*)&pixels[col][row*3+2], 1);
      outfile.write((const char*)&pixels[col][row*3+1], 1);
      outfile.write((const char*)&pixels[col][row*3+0], 1);
    }
